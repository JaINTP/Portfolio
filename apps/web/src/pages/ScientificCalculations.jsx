...
import { FlaskConical, Scale, RotateCcw, Waves, Beaker, Zap, Activity, Info, BookOpen, Lightbulb } from 'lucide-react';
        ...
        <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          <div className="max-w-6xl mx-auto space-y-10">
            ...
            <Tabs defaultValue="prep" className="w-full">
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            ...
                            <div className="bg-cyan-500/5 border border-cyan-500/10 p-6 rounded-3xl space-y-4">
                                <div className="flex items-center space-x-2 text-cyan-400">
                                    ...
                                    <span className="text-[10px] font-black uppercase tracking-widest">Guide: Preparation</span>
                                </div>
                                <p className="text-xs text-gray-500 leading-relaxed font-medium">
                                    To find the <span className="text-white">pH of a solution</span>, select a substance and enter its Molarity. Use the <span className="text-cyan-400 font-bold">Dilution toggle</span> to simulate moving an aliquot from a stock bottle. To find <span className="text-white">how much to weigh</span>, switch to <span className="text-white">pH → Conc</span> and enter your target volume.
                                </p>
                            </div>
                        </div>
              ...
              {/* TAB: Buffer Solver */}
              <TabsContent value="buffer" className="outline-none animate-in fade-in duration-500 space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-5 space-y-6">
                        <Card className="bg-white/[0.02] border-white/5 rounded-3xl p-8 space-y-8 shadow-inner">
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <Input type="number" value={bufferInput.acidConc} onChange={e=>setBufferInput({...bufferInput, acidConc: e.target.value})} className="h-12 bg-black border-white/10 font-bold rounded-xl" />
                                    </div>
                                    <div className="space-y-2">
                                        <Label className="text-[10px] font-black uppercase text-gray-500">[Base] Concentration</Label>
                                        <Input type="number" value={bufferInput.baseConc} onChange={e=>setBufferInput({...bufferInput, baseConc: e.target.value})} className="h-12 bg-black border-white/10 font-bold rounded-xl" />
                                    </div>
                                </div>
                            </div>
                            <div className="pt-8 border-t border-white/5 text-gray-500 text-[10px] uppercase font-black tracking-widest">
                                Formula: <InlineMath math="pH = pK_a + \log\frac{[Base]}{[Acid]}" />
                            </div>
                        </Card>

                        {/* Tutorial: Buffer Solver */}
                        <div className="bg-white/5 border border-white/10 p-6 rounded-3xl space-y-4">
                            <div className="flex items-center space-x-2 text-white">
                                <Lightbulb className="w-4 h-4 text-cyan-400" />
                                <span className="text-[10px] font-black uppercase tracking-widest">Scientific Insight</span>
                            </div>
                            <p className="text-xs text-gray-500 leading-relaxed font-medium">
                                A buffer resists pH changes. When <span className="text-white">[Acid] = [Base]</span>, the <span className="text-cyan-400 font-bold">pH equals the pKa</span>. Use this solver to determine the final pH of a mixture or to calculate the concentration ratio needed to hit a target pH for your experiment.
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-7">
                        {bufferResult ? (
                            <Card className="bg-white/[0.03] border-white/5 rounded-3xl h-full p-10 flex flex-col justify-center items-center relative overflow-hidden shadow-2xl">
                                ...
                                <div className="px-6 py-2 bg-white/5 border border-white/10 rounded-full text-[10px] font-black uppercase tracking-widest text-gray-400">
                                    Conjugate Base/Acid Ratio: <span className="text-white ml-2">{bufferResult.ratio.toFixed(3)}</span>
                                </div>
                            </Card>
                        ) : (
              ...
              {/* TAB: Beer-Lambert */}
              <TabsContent value="optics" className="outline-none animate-in fade-in duration-500 space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
                                {['abs', 'eps', 'path', 'conc'].map(f => (
                                    <div key={f} className="space-y-3">
                                        ...
                                        <Input 
                                            placeholder={f === 'path' ? '1.0 cm' : 'Solve for this...'} 
                                            type="number" 
                                            value={beerInput[f]} 
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
                                ...
                                <div className="bg-white/5 border border-white/10 p-8 rounded-[30px] space-y-4">
                                    <div className="flex items-center space-x-2 text-cyan-400">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest">Tutorial: Spectroscopy</span>
                                    </div>
                                    <p className="text-xs text-gray-500 leading-relaxed font-medium">
                                        This tool solves for any unknown in the Beer-Lambert Law. Enter 3 values, and leave the one you want to solve for <span className="text-white">blank</span>. Standard path length is usually <span className="text-white">1.0 cm</span>. Ensure your Molar Absorptivity unit matches your concentration unit.
                                    </p>
                                </div>
                            </div>
                        </Card>
              ...
              <TabsContent value="assay" className="outline-none animate-in fade-in duration-500">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    <div className="lg:col-span-5 space-y-6">
                        <Card className="bg-white/[0.02] border-white/5 rounded-3xl p-10 space-y-8 shadow-inner">
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <Input type="number" value={assayInput.density} onChange={e=>setAssayInput({...assayInput, density: e.target.value})} className="h-12 bg-black border-white/10 font-bold rounded-xl" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black text-gray-500 uppercase">Assay (wt %)</Label>
                                    <Input type="number" value={assayInput.percent} onChange={e=>setAssayInput({...assayInput, percent: e.target.value})} className="h-12 bg-black border-white/10 font-bold rounded-xl" />
                                </div>
                                <div className="space-y-2">
                                    <Label className="text-[10px] font-black text-gray-500 uppercase">Molecular Weight (g/mol)</Label>
                                    <Input type="number" value={assayInput.mw} onChange={e=>setAssayInput({...assayInput, mw: e.target.value})} className="h-12 bg-black border-white/10 font-bold rounded-xl" />
                                </div>
                            </div>
                        </Card>

                        {/* Tutorial: Stock Assay */}
                        <div className="bg-white/5 border border-white/10 p-8 rounded-3xl space-y-4">
                            <div className="flex items-center space-x-2 text-cyan-400">
                                <FlaskConical className="w-4 h-4" />
                                <span className="text-[10px] font-black uppercase tracking-widest">Tutorial: Stock Bottles</span>
                            </div>
                            <p className="text-xs text-gray-500 leading-relaxed font-medium">
                                Commercial acids (like 37% HCl) are sold by weight percent, not molarity. Use this tool to find the <span className="text-white">exact Molarity</span> of your stock bottle. You can find Density ($\rho$) and Assay (%) on the bottle label or the manufacturer&apos;s Certificate of Analysis (CoA).
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-7">
                        {assayResult && (
                            <Card className="bg-white/[0.03] border-white/5 rounded-3xl h-full p-10 flex flex-col justify-center relative overflow-hidden shadow-2xl">
                                <Waves className="absolute right-0 top-0 w-64 h-64 text-cyan-500 opacity-5 -mr-20 -mt-20" />
                                <p className="text-[10px] font-black uppercase tracking-[0.3em] text-cyan-500 mb-4 text-center">Stock Bottle Molarity</p>
                                <p className="text-[140px] font-black tracking-tighter mb-4 text-center leading-none">
                                    {assayResult.toFixed(2)}
                                    <span className="text-2xl text-gray-600 ml-4 font-bold uppercase">M</span>
                                </p>
                                <div className="mt-8 bg-black/40 p-6 rounded-2xl border border-white/5 space-y-4">
                                    <BlockMath math={`M = \\frac{\\rho \\cdot Assay\\% \\cdot 10}{MW}`} />
                                </div>
                            </Card>
                        )}
                    </div>
            ...
            <div className="pt-10 border-t border-white/5 grid md:grid-cols-2 gap-10">
                <div className="flex items-start space-x-4">
                    <div className="p-3 bg-white/5 rounded-xl border border-white/10 shadow-inner"><Info className="w-5 h-5 text-gray-400" /></div>
                    <div className="space-y-2">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-300">Analytical Precision</h4>
                        <p className="text-xs text-gray-500 leading-relaxed font-medium">Calculations utilize exact quadratic equilibrium solvers at 25°C. For high-molarity solutions, physical activity coefficients may deviate from these theoretical models.</p>
                    </div>
                </div>
                <div className="flex items-start space-x-4">
                    <div className="p-3 bg-white/5 rounded-xl border border-white/10 shadow-inner"><Beaker className="w-5 h-5 text-gray-400" /></div>
                    <div className="space-y-2">
                        <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-300">Safety Protocols</h4>
                        <p className="text-xs text-gray-500 leading-relaxed font-medium">Always cross-reference calculations with experimental measurements. Consult Safety Data Sheets (SDS) before preparing concentrated reagents.</p>
                    </div>
                </div>
            </div>
          </div>
