...
export default function ScientificCalculations() {
  const [activeTab, setActiveTab] = useState('prep')

  const Tutorial = ({ title, steps, formula, explanation }: any) => (
  ...
  const [substanceResult, setSubstanceResult] = useState<any>(null)
  ...
  // --- TAB 3: Spectroscopy ---
  const [beerInput, setBeerInput] = useState<any>({ abs: '', eps: '', path: '1', conc: '' })
  const [beerResult, setBeerResult] = useState<any>(null)

  // --- TAB 4: Stock Assay ---
  const [assayInput, setAssayInput] = useState({ density: '1.19', percent: '37', mw: '36.46' })
  const [assayResult, setAssayResult] = useState<any>(null)

  const solveQuadratic = (a: number, b: number) => {
    if (b === 0) return 0
  ...
  useEffect(() => {
    ...
    if (calcMode === 'find_ph') {
        ...
        else if (oh) { poh = -Math.log10(oh); ph = 14 - poh; h = Math.pow(10, -ph); }
        // @ts-ignore
      ...
      <div className="max-w-6xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-10">
            <div className="space-y-4">
                ...
                <h1 className="text-5xl md:text-7xl font-black tracking-tighter text-white">Lab Calc<span className="text-cyan-500">.</span></h1>
        ...
        <div className="w-full">
          <div className="grid w-full grid-cols-2 lg:grid-cols-4 bg-white/5 border border-white/10 p-1 mb-10 rounded-2xl">
            <button 
              onClick={() => setActiveTab('prep')}
              className={`rounded-xl font-black text-[10px] uppercase tracking-widest py-3 transition-all ${activeTab === 'prep' ? 'bg-cyan-500 text-black' : 'text-gray-400 hover:text-white'}`}
            >
              Preparation
            </button>
            <button 
              onClick={() => setActiveTab('buffer')}
              className={`rounded-xl font-black text-[10px] uppercase tracking-widest py-3 transition-all ${activeTab === 'buffer' ? 'bg-cyan-500 text-black' : 'text-gray-400 hover:text-white'}`}
            >
              Buffer Solver
            </button>
            <button 
              onClick={() => setActiveTab('optics')}
              className={`rounded-xl font-black text-[10px] uppercase tracking-widest py-3 transition-all ${activeTab === 'optics' ? 'bg-cyan-500 text-black' : 'text-gray-400 hover:text-white'}`}
            >
              Spectroscopy
            </button>
            <button 
              onClick={() => setActiveTab('assay')}
              className={`rounded-xl font-black text-[10px] uppercase tracking-widest py-3 transition-all ${activeTab === 'assay' ? 'bg-cyan-500 text-black' : 'text-gray-400 hover:text-white'}`}
            >
              Stock Assay
            </button>
          </div>
                            ...
                            <div className="space-y-2 text-left">
                                ...
                                <select value={substanceId} onChange={e=>setSubstanceId(e.target.value)} className="w-full h-12 px-4 rounded-2xl bg-black border border-white/10 text-white font-bold focus:border-cyan-500 outline-none appearance-none">
                                    ...
                                    <optgroup label="Salts & Buffers" className="bg-gray-900">
                                        {SUBSTANCES.filter(s => s.type.includes('salt') || (s.pka && s.pka > 0)).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                                    </optgroup>
                                </select>
                            </div>
                            ...
                            <div className="space-y-2 text-left">
                                ...
                                {useDilutionSource && calcMode === 'find_ph' ? (
                                    <div className="grid grid-cols-3 gap-2">
                                        <input placeholder="Stock M" type="number" value={stockVal.c} onChange={e=>setStockVal({...stockVal, c: e.target.value})} className="h-11 bg-black border border-white/10 rounded-xl px-2 text-xs font-bold text-white outline-none focus:border-cyan-500" />
                                        <input placeholder="Aliq. L" type="number" value={stockVal.v} onChange={e=>setStockVal({...stockVal, v: e.target.value})} className="h-11 bg-black border border-white/10 rounded-xl px-2 text-xs font-bold text-white outline-none focus:border-cyan-500" />
                                        <input placeholder="Total L" type="number" value={stockVal.totalV} onChange={e=>setStockVal({...stockVal, totalV: e.target.value})} className="h-11 bg-black border border-white/10 rounded-xl px-2 text-xs font-bold text-white outline-none focus:border-cyan-500" />
                                    </div>
                                ) : (
                                    <input type="number" value={inputVal} onChange={e=>setInputVal(e.target.value)} className="w-full h-12 bg-black border border-white/10 font-bold text-lg rounded-2xl text-white px-4 outline-none focus:border-cyan-500" placeholder={calcMode === 'find_ph' ? 'Molarity' : 'pH Level'} />
                                )}
                            </div>
                            <div className="space-y-2 text-left">
                                <label className="text-[10px] uppercase font-black text-gray-500 ml-1">Preparation Volume (L)</label>
                                <input type="number" value={targetVolume} onChange={e=>setTargetVolume(e.target.value)} className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl text-white px-4 outline-none focus:border-cyan-500" />
                            </div>
                        </div>
                    </div>
                    <div className="lg:col-span-8 space-y-6">
                        {substanceResult ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in slide-in-from-right-4 duration-500">
                                <div className="bg-white/[0.03] border border-white/5 rounded-3xl p-8 relative overflow-hidden shadow-2xl text-left">
                                    <p className="text-[10px] uppercase tracking-[0.3em] text-cyan-500 font-black mb-2">Analysis Result</p>
                                    <p className="text-7xl font-black tracking-tighter text-white">pH {substanceResult.ph.toFixed(3)}</p>
                                ...
                                <div className="bg-cyan-500 border-none rounded-3xl p-8 relative overflow-hidden shadow-2xl shadow-cyan-500/20 text-left">
                                ...
                                <div className="md:col-span-2 bg-white/[0.01] border border-white/5 rounded-3xl p-8 text-left">
                                    ...
                                    <div className="space-y-4">
                                        {substanceResult.steps.map((s: string, i: number) => <div key={i} className="bg-black/40 p-4 rounded-2xl border border-white/5 font-mono"><BlockMath math={s} /></div>)}
                                        {substanceResult.labPrep && <div className="bg-cyan-500/5 p-4 rounded-2xl border border-cyan-500/10 font-mono text-white"><BlockMath math={`m = ${substanceResult.labPrep.m.toExponential(3)} \\text{ M} \\cdot ${substanceResult.labPrep.vol} \\text{ L} \\cdot ${substanceResult.labPrep.mw} \\text{ g/mol}`} /></div>}
                                    </div>
                                </div>
                            </div>
                        ) : (
                      ...
                      <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 space-y-8">
                          <div className="space-y-6">
                              <div className="space-y-2">
                                  ...
                                  <input type="number" value={bufferInput.pka} onChange={e=>setBufferInput({...bufferInput, pka: e.target.value})} className="w-full h-12 bg-black border border-white/10 font-bold text-xl rounded-2xl text-cyan-400 px-4 outline-none focus:border-cyan-500" />
                              </div>
                              <div className="grid grid-cols-2 gap-6">
                                  <div className="space-y-2">
                                      <label className="text-[10px] font-black uppercase text-gray-500">[Acid] Concentration</label>
                                      <input type="number" value={bufferInput.acidConc} onChange={e=>setBufferInput({...bufferInput, acidConc: e.target.value})} className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl text-white px-4 outline-none focus:border-cyan-500" />
                                  </div>
                                  <div className="space-y-2">
                                      <label className="text-[10px] font-black uppercase text-gray-500">[Base] Concentration</label>
                                      <input type="number" value={bufferInput.baseConc} onChange={e=>setBufferInput({...bufferInput, baseConc: e.target.value})} className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl text-white px-4 outline-none focus:border-cyan-500" />
                                  </div>
                              </div>
                          </div>
                  ...
                  <div className="lg:col-span-7">
                      {bufferResult ? (
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                              <div className="bg-cyan-500 text-black border-none rounded-3xl p-10 flex flex-col justify-center items-center relative overflow-hidden shadow-2xl shadow-cyan-500/20">
                                  <Activity className="absolute right-[-5%] bottom-[-5%] w-48 h-48 text-black/10 rotate-12" />
                                  <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-4 opacity-60">Calculated Buffer pH</p>
                                  <p className="text-[120px] font-black leading-none tracking-tighter mb-6">{bufferResult.ph.toFixed(2)}</p>
                                  <div className="px-6 py-2 bg-black/10 border border-black/10 rounded-full text-[10px] font-black uppercase tracking-widest">
                                      Base/Acid Ratio: {bufferResult.ratio.toFixed(3)}
                                  </div>
                              </div>
                          </div>
                      ) : (
                          <div className="h-full min-h-[300px] border-2 border-dashed border-white/5 rounded-[40px] flex flex-col items-center justify-center text-gray-700">
                              <Activity className="w-16 h-12 opacity-10 mb-4" />
                              <span className="text-[10px] font-black uppercase tracking-widest">Waiting for buffer parameters...</span>
                          </div>
                      )}
                  </div>
              </div>
                              ...
                              {['abs', 'eps', 'path', 'conc'].map(f => (
                                  <div key={f} className="space-y-2">
                                      ...
                                      <input 
                                          ...
                                          className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl focus:border-cyan-500 text-white px-4 outline-none" 
                                      />
                                  </div>
                              ))}
                          </div>
                          <div className="pt-4 border-t border-white/5">
                               <p className="text-[9px] font-bold text-gray-600 uppercase tracking-widest italic text-center">A = ε · b · c</p>
                          </div>
                      </div>
                  </div>
                  <div className="lg:col-span-7">
                      {beerResult ? (
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                              <div className="bg-cyan-500 text-black border-none rounded-3xl p-10 relative overflow-hidden shadow-2xl shadow-cyan-500/20">
                                  <Zap className="absolute right-[-5%] bottom-[-5%] w-48 h-48 text-black/10 rotate-12" />
                                  <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-2 opacity-60 text-center">Calculated Parameter</p>
                                  <h3 className="text-2xl font-black mb-2 text-center text-black">{beerResult.label}</h3>
                                  <p className="text-7xl font-black tracking-tighter text-center leading-none text-black">{beerResult.res.toExponential(4)}</p>
                              </div>
                          </div>
                      ) : (
                          <div className="h-full min-h-[400px] border-2 border-dashed border-white/5 rounded-[40px] flex flex-col items-center justify-center text-gray-700 space-y-4">
                              <Zap className="w-16 h-12 opacity-10" />
                              <p className="text-[10px] font-black uppercase tracking-widest">Awaiting optical data inputs</p>
                          </div>
                      )}
                  </div>
              </div>
                      ...
                      <div className="bg-white/[0.02] border border-white/5 rounded-3xl p-8 space-y-8">
                          ...
                          <div className="grid grid-cols-1 gap-6">
                              <div className="space-y-2">
                                  ...
                                  <input type="number" value={assayInput.density} onChange={e=>setAssayInput({...assayInput, density: e.target.value})} className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl text-white px-4 outline-none focus:border-cyan-500" />
                              </div>
                              <div className="space-y-2">
                                  <label className="text-[10px] font-black text-gray-500 uppercase">Assay (wt %)</label>
                                  <input type="number" value={assayInput.percent} onChange={e=>setAssayInput({...assayInput, percent: e.target.value})} className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl text-white px-4 outline-none focus:border-cyan-500" />
                              </div>
                              <div className="space-y-2">
                                  <label className="text-[10px] font-black text-gray-500 uppercase">Molecular Weight (g/mol)</label>
                                  <input type="number" value={assayInput.mw} onChange={e=>setAssayInput({...assayInput, mw: e.target.value})} className="w-full h-12 bg-black border border-white/10 font-bold rounded-2xl text-white px-4 outline-none focus:border-cyan-500" />
                              </div>
                          </div>
                      </div>
                  </div>
                      ...
                      {assayResult ? (
                          <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
                              <div className="bg-cyan-500 text-black border-none rounded-3xl p-10 flex flex-col justify-center items-center relative overflow-hidden shadow-2xl shadow-cyan-500/20">
                                  ...
                                  <p className="text-[10px] font-black uppercase tracking-[0.3em] mb-4 opacity-60">Calculated Stock Molarity</p>
                                  <p className="text-[100px] font-black tracking-tighter mb-4 leading-none text-black">{assayResult.toFixed(2)}<span className="text-2xl ml-4 font-bold">M</span></p>
                              </div>
                          </div>
                      ) : (
                          <div className="h-full min-h-[300px] border-2 border-dashed border-white/5 rounded-[40px] flex flex-col items-center justify-center text-gray-700">
                              <Waves className="w-16 h-12 opacity-10 mb-4" />
                              <span className="text-[10px] font-black uppercase tracking-widest">Awaiting commercial bottle data...</span>
                          </div>
                      )}
                  </div>
              </div>
        ...
        {/* Global Guidelines */}
        <div className="pt-10 border-t border-white/5 grid md:grid-cols-2 gap-10 text-left">
            <div className="flex items-start space-x-4">
                <div className="p-3 bg-white/5 rounded-xl border border-white/10"><Info className="w-5 h-5 text-gray-400" /></div>
                <div className="space-y-2">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-300">Thermodynamic Consistency</h4>
                    <p className="text-xs text-gray-500 leading-relaxed font-medium">Calculations are modeled for 25°C using exact quadratic solvers. For high-ionic strength solutions ( {'>'} 0.1M), activity coefficients may slightly alter observed pH.</p>
                </div>
            </div>
            <div className="flex items-start space-x-4">
                <div className="p-3 bg-white/5 rounded-xl border border-white/10"><Beaker className="w-5 h-5 text-gray-400" /></div>
                <div className="space-y-2">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-gray-300">Laboratory Safety</h4>
                    <p className="text-xs text-gray-500 leading-relaxed font-medium">Always verify theoretical results against experimental readings. Consult SDS before handling concentrated stocks calculated in the Assay tab.</p>
                </div>
            </div>
        </div>
      </div>
